...
def _split_data(x_data, y_data=None, train_ratio=0, split_type="sequential"):
...
#             (data_train, data_test) = _split_data(
#                 data_df["EventSequence"].values,
#                 data_df["Label"].values,
#                 train_ratio,
#                 split_type,
#             )
#             (x_train, y_train), (_, _) = data_train
...
#             print("xxx", x_data)
#             (data_train, _), (data_test, _) = _split_data(
...
def load_HDFS(
    ...
    if log_file.endswith(".npz"):
        ...
        struct_log = pd.read_csv(log_file, engine="c", na_filter=False, memory_map=True)
        data_dict = OrderedDict()
        # print("struct_log", struct_log)
        for idx, row in struct_log.iterrows():
            # blkId_list = re.findall(r"(blk_-?\d+)", row["Content"])
            blkId_list = re.findall(r"^\.{3,}\w+_?\w*?:\s\w+:\s.+", row["Content"])
            blkId_set = set(blkId_list)
            # print("blkId_set", blkId_list)
            for blk_Id in blkId_set:
                if not blk_Id in data_dict:
                    data_dict[blk_Id] = []
                data_dict[blk_Id].append(row["EventId"])
